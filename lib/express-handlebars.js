...
ExpressHandlebars.prototype.renderView = function (viewPath, options, callback) {
    ...
    if (viewsPath) {
        viewsPath = this._getViewsPath(viewsPath, viewPath);
...
ExpressHandlebars.prototype._getViewsPath = function(viewsPath, viewPath) {
  if (!Array.isArray(viewsPath)) {
     return viewsPath;
  }

  var k = 0;
  var len = viewsPath.length;
  var normalizeViewPath = path.normalize(viewPath);
  while (k < len) {
    if(normalizeViewPath.indexOf(viewsPath[k]) > - 1) {
        return viewsPath[k];
    }
    k++;
  }
