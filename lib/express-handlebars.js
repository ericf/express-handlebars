...
ExpressHandlebars.prototype._getView = function(viewsPath, viewPath) {
  ...
  var normalizeViewPath = path.normalize(viewPath).replace(stripFilename, '');
  while (k < len) {
    if(viewsPath[k] === normalizeViewPath) {
        return this._getTemplateName(path.relative(viewsPath[k], viewPath));
    }
    k++;
  }
}
